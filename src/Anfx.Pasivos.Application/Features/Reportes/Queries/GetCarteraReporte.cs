...
public class GetCarteraActivaPorVencer : IQuery<Result<PagedResultDto<CarteraReporteDto>>>
{
...
internal class GetCarteraActivaPorVencerHandler : IQueryHandler<GetCarteraActivaPorVencer, Result<PagedResultDto<CarteraReporteDto>>>
{
    private readonly IApplicationDbContext _context;
    private readonly IPaginator _paginator;
    private readonly IDynamicSorter _sorter;

    public GetCarteraActivaPorVencerHandler(IApplicationDbContext context, IPaginator paginator, IDynamicSorter sorter)
    {
        _context = context;
        _paginator = paginator;
        _sorter = sorter;
    }
    public async Task<Result<PagedResultDto<CarteraReporteDto>>> HandleAsync(GetCarteraActivaPorVencer request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var query = _context.View_CarteraActiva_PorVencer.AsQueryable();



            if (!String.IsNullOrEmpty(request.SearchText))
            {
                query = query.Where(w => w.Contrato.Contains(request.SearchText));
            }
            ...
            var result = await _paginator.PaginateAsync<View_CarteraActiva_PorVencer, CarteraReporteDto>(
                sortedQuery,
                request.Page,
                request.PageSize
            );
