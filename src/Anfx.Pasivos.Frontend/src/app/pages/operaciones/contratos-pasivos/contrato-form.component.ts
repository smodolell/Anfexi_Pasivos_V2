...
export class ContratoFormComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    capital:          [null as number | null, Validators.required],
    capitalFinanciado:[null as number | null],
  ...
  get pagosArray(): FormArray { return this.form.get('pagos') as FormArray; }
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      const state = (nav?.extras.state ?? history.state) as { editDto?: ContratoPasivoEditDto; lineaCreditoNombre?: string } | undefined;
      if (state?.editDto) {
    ...
    this.form.get('idTasa')!.valueChanges.subscribe(idTasa => {
      if (idTasa == null) { this.form.patchValue({ tasa: null, tasaBase: null }, { emitEvent: false }); return; }
    ...
    this.form.get('idTasaMora')!.valueChanges.subscribe(idTasaMora => {
      if (idTasaMora == null) { this.form.patchValue({ tasaMora: null, tasaBaseMora: null }, { emitEvent: false }); return; }
    ...
    this.form.get('idTipoTablaAmortiza')!.valueChanges.subscribe(id => {
      ...
      this.form.patchValue({ idTipoCapitalizacion: null, idTipoPagoCapital: null, idPeriodicidad_TTA: null, noPagosIrregulares: null }, { emitEvent: false });
    ...
    op$.subscribe({
      next: (res: any) => {
        ...
        if (res.success === false) {
          ...
          const msg = this.isEditMode() ? 'Contrato actualizado correctamente' : 'Contrato creado correctamente';
          this.utilsSvc.showNotification('Éxito', msg, 'success');
      ...
      next: (res) => {
        ...
        if (res.success && res.data) {
          ...
          this.form.patchValue({
            ...
            estatusContrato:     this.estatusContratoList().find(e => e.value === d.idEstatusContrato)?.text ?? '',
          ...
          if (d.idTipoTablaAmortiza) {
            this.cargarInfoTablaAmortiza(d.idTipoTablaAmortiza, d.idTipoCapitalizacion, d.idTipoPagoCapital, d.idPeriodicidad_TTA);
          }

          if (d.pagos?.length) {
            d.pagos.forEach(p => this.pagosArray.push(this.fb.group({
  ...
  private populateForm(d: ContratoPasivoEditDto, lineaCreditoNombre?: string): void {
    ...
    this.form.patchValue({
      fondeador:           d.fondeador ?? '',
      lineaCreditoNombre:  lineaCreditoNombre ?? '',
      maxCapitalDisponible:d.maxCapitalDisponible ?? null,
      idTipoCredito:       d.idTipoCredito ?? null,
      contrato:            d.contrato ?? '',
      capitalFinanciado:   d.capitalFinanciado ?? null,
...
nroRentasDepositoGarantia: v.nroRentasDepositoGarantia ?? undefined,
      ...
      pagos: this.pagosArray.length > 0
  ...
  private toDateInput(val?: string | null): string {
    if (!val) return '';
