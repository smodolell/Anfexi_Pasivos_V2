...
export class TipoCreditoListComponent implements OnInit {
  ...
  items       = signal<TipoCreditoListItemDto[]>([]);
  ...
  onNuevo() {
    this.router.navigate(['/configuracion/tipo-credito/new']);
  }

  onAction(event: TableActionEvent<TipoCreditoListItemDto>) {
    if (event.action === 'edit') {
      this.router.navigate(['/configuracion/tipo-credito/edit', event.row.id]);
    }
  }
    ...
    this.service.getAll({
      ...
      next: (res) => {
        if (res.success && res.data) {
          ...
          this.utilsService.showNotification('Error', 'Error al cargar tipos de crédito', 'error');
        }
